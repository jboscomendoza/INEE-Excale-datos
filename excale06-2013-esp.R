#Este documento contiene código para descargar, leer, guardar y manipular en R la base de datos correspondientes al EXALE de Español del EXCALE 06, aplicación 2013.

#1. Descarga
#Para la descarga y del archivo zip que contiene la base de datos del EXCALE de Español se pueden seguir dos procedimiento.
...
#Liberación del archivo temporal asignado a temp
unlink(temp)
rm(temp)

esp <- read_fwf(unz(temp, "Excale06_2014_Alum_Esp.TXT"), fwf_widths(anchos, nombres))

#2.3 Recodificación de valores numéricos que indican datos perdidos a NA (opcional)
...
rm(esp01, esp02, esp03, esp04, esp05, esp06, esp07, esp08, esp09, esp10)
