...
temp <- tempfile()
download.file("http://www.inee.edu.mx/images/stories/documentos_pdf/Bases_Datos/EXCALE2007_Pree3/excale00_2007_alum_pm.zip", temp)

#Procedimiento 2: Descarga del archivo zip y extracción de la base de datos al directorio de trabajo.
download.file("http://www.inee.edu.mx/images/stories/documentos_pdf/Bases_Datos/EXCALE2007_Pree3/excale00_2007_alum_pm.zip", "excale00_2007_alum_pm.zip")
unzip("excale00_2007_alum_pm.zip", files = "Excale00_2007_Alum_PM.TXT")
...
p_m <- read.fwf(unz(temp, "Excale00_2007_Alum_PM.TXT"), widths = anchos, col.names = c(nombres))
#Si se ha descargado la base de datos al directorio de trabajo usar:
p_m <- read.fwf("Excale00_2007_Alum_PM.TXT", widths = anchos, col.names = c(nombres))
...
p_m <- read_fwf(unz(temp, "Excale00_2007_Alum_PM.TXT"), fwf_widths(anchos, c(nombres)))
#Si se ha descargado la base de datos al directorio de trabajo usar:
p_m <- read_fwf("Excale00_2007_Alum_PM.TXT", widths = anchos, col.names = c(nombres))
...
p_m03 <- subset(p_m, select = c(EDAD))
p_m04 <- subset(p_m, select = c(EDADC))
p_m05 <- subset(p_m, select = c(EDAD1SEP))
p_m06 <- subset(p_m, select = c(EDAD1SEPC:EDADN))
p_m07 <- subset(p_m, select = c(EDAD_AC:MES_NAC))
p_m08 <- subset(p_m, select = c(W_ALU))
p_m09 <- subset(p_m, select = c(DOCENTE))
p_m10 <- subset(p_m, select = c(EMA_01:EMD_10))
p_m11 <- subset(p_m, select = c(WVARSTRR:W_FSTR80))
...
p_m06[p_m06 == 9] <- NA
p_m07[p_m07 > 97] <- NA
p_m09[p_m09 == 99999] <- NA
p_m10[p_m10 > 4] <- NA

p_m <- cbind(p_m01, p_m02, p_m03, p_m04, p_m05, p_m06, p_m07, p_m08, p_m09, p_m10, p_m11)
rm(p_m01, p_m02, p_m03, p_m04, p_m05, p_m06, p_m07, p_m08, p_m09, p_m10, p_m11, anchos, nombres)

#III. Escritura
#Escritura de archivo csv que permite una lectura más sencilla de la base de datos.
#Procedimiento 1: Con librerías base
#Escritura
write.csv(p_m, "excale00_2007_p_m.csv")
#Lectura a variable p_m
p_m <- read.csv("excale00_2007_p_m.csv")

#Procedimiento 2: Con readr
#Escritura
write_csv(p_m, "excale00_2007_p_m.csv")
#Lectura a variable p_m
p_m <- read_csv("excale00_2007_p_m.csv")
