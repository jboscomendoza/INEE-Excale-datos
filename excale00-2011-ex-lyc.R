...
temp <- tempfile()
download.file("http://www.inee.edu.mx/images/stories/documentos_pdf/Bases_Datos/EXCALE00_2011/excale00_2011_alum_lc.zip", temp)
...
lyc <- read.fwf(unz(temp, "Excale00_2011_Alum_LC.TXT"), widths = anchos, col.names = nombres)
#Si se ha descargado la base de datos al directorio de trabajo usar:
lyc <- read.fwf("Excale00_2011_Alum_LC.TXT", widths = anchos, col.names = nombres)
...
lyc <- read_fwf(unz(temp, "Excale00_2011_Alum_LC.TXT"), fwf_widths(anchos, nombres))
#Si se ha descargado la base de datos al directorio de trabajo usar:
...
lyc03 <- subset(lyc, select = c(FECHA_NAC))
lyc04 <- subset(lyc, select = c(EDAD))
lyc05 <- subset(lyc, select = c(EDADC:EDADN))
lyc06 <- subset(lyc, select = c(EDAD_AC))
lyc07 <- subset(lyc, select = c(W_ALU))
lyc08 <- subset(lyc, select = c(ELA_01:ELD_10))
lyc09 <- subset(lyc, select = c(PV1ESP:W_FSTR80))
...
lyc <- cbind(lyc01, lyc02, lyc03, lyc04, lyc05, lyc06, lyc07, lyc08, lyc09)
rm(lyc01, lyc02, lyc03, lyc04, lyc05, lyc06, lyc07, lyc08, lyc09, anchos, nombres)

#III. Escritura
#Escritura de archivo csv que permite una lectura más sencilla de la base de datos.
#Procedimiento 1: Con librerías base
#Escritura
write.csv(lyc, "excale00_2011_ex-lyc.csv")
#Lectura a variable lyc
lyc <- read.csv("excale00_2011_ex-lyc.csv")
